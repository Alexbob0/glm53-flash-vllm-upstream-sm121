...
# [glm53-loadclone] helpers (see safetensors_weights_iterator)
def _glm53_load_clone() -> bool:
    return os.environ.get("GLM53_LOAD_CLONE", "1") != "0"


def _glm53_load_prefetch_depth() -> int:
    """Number of shards read ahead into the page cache by background threads (0 = off)."""
    try:
        return max(0, int(os.environ.get("GLM53_LOAD_PREFETCH", "0") or 0))
    except ValueError:
        return 0


def _glm53_read_whole_file(path: str) -> None:
    """Sequential read into the page cache; the bytes are discarded."""
    try:
        with open(path, "rb", buffering=0) as fh:
            buf = bytearray(32 << 20)
            while fh.readinto(buf):
                pass
    except OSError as exc:  # never fail the load for a prefetch
...
class _Glm53Prefetcher:
    ...
    def __init__(self, files: list[str], depth: int):
        self._files = files
        self._depth = depth
        self._pool = concurrent.futures.ThreadPoolExecutor(
            max_workers=depth, thread_name_prefix="glm53-prefetch"
        )
        self._futures: dict[int, concurrent.futures.Future] = {}
        self._next = 0
        logger.info("[glm53-loadclone] prefetch: %d shards ahead on %d threads", depth, depth)

    def advance(self, idx: int) -> None:
        for j in range(self._next, min(idx + 1 + self._depth, len(self._files))):
            self._futures[j] = self._pool.submit(_glm53_read_whole_file, self._files[j])
        self._next = max(self._next, min(idx + 1 + self._depth, len(self._files)))
...
def safetensors_weights_iterator(
    ...
    *,
    ...
    _glm53_prefetcher = (
        _Glm53Prefetcher(sorted_files, _glm53_load_prefetch_depth())
        if _glm53_load_prefetch_depth() > 0 and safetensors_load_strategy != "eager"
        else None
    )
    for _file_idx, st_file in enumerate(tqdm(
        sorted_files,
        desc=loading_desc,
        disable=not enable_tqdm(use_tqdm_on_load),
        bar_format=_BAR_FORMAT,
        ...
        if safetensors_load_strategy == "eager":
            ...
            # [glm53-loadclone] GB10/UMA: a host->device copy whose source is a
            # file-backed mmap tensor runs at ~0.1 GB/s under a CUDA context
            # (measured 2026-09-21: 63 s for 5.2 GiB vs 2.6 s once cloned to
            # anonymous memory). Materialize each tensor on the CPU first; the
            # bytes are identical, only the copy path changes. Optionally ask the
            # kernel to read the next shard ahead while this one is consumed.
            if _glm53_prefetcher is not None:
                _glm53_prefetcher.advance(_file_idx)
            with safe_open(st_file, framework="pt") as f:
                for name in f.keys():  # noqa: SIM118
                    if should_skip_weight(name, local_expert_ids):
                        continue
