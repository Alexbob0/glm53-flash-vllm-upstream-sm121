#!/usr/bin/env python3
"""[glm53-upstream nightly port, 24/09] of MiaAI overlay/patch_mamba_align_state_free.py (fd329d2).
Only change vs MiaAI: the align reservation keeps the nightly's num_prefill_checkpoint_blocks term.
Generate: GLM53_SINGLE_TYPE_KV_CACHE_MANAGER_PY=<copy> GLM53_KV_CACHE_INTERFACE_PY=<copy> python3 this.py
(see build.sh; run.sh mounts the outputs RO when MAMBA_FREE=1).
