...
def _install_exllamav3_namespace() -> None:
    ...
    if "exllamav3.model.config" not in sys.modules:
        # exllamav3 >= 1.4: LinearEXL3(config=None) instantiates NullConfig, whose
        # InferParams carries the MGEMM/int8-GEMV thresholds read by forward(). The real
        # config.py imports without flash_attn; minimal stub as a fallback only.
        try:
