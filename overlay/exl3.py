...
def _install_exllamav3_namespace() -> None:
    ...
    if "exllamav3.model.config" not in sys.modules:
        # exllamav3 >= 1.4 : LinearEXL3(config=None) instancie NullConfig, dont
        # InferParams porte les seuils MGEMM/int8-GEMV lus par forward(). Le vrai
        # config.py s'importe sans flash_attn ; stub minimal en repli seulement.
        try:
            importlib.import_module("exllamav3.model.config")
        except ImportError:
            ...
            config.NullConfig = type(
                ...
                {"__init__": lambda self: setattr(
                    self, "infer_params", types.SimpleNamespace(no_reconstruct=False))},
            )
...
_FAT_STREAM_CACHE: dict = {}


def _fat_streams(device: torch.device, n: int) -> list:
    key = (str(device), n)
    streams = _FAT_STREAM_CACHE.get(key)
    if streams is None:
        streams = [torch.cuda.Stream(device=device) for _ in range(n)]
        _FAT_STREAM_CACHE[key] = streams
    return streams


def fat_streams_requested() -> int:
    try:
        return max(1, int(os.environ.get("EXL3_FAT_STREAMS", "1") or 1))
    except ValueError:
        return 1
...
def _fat_scratch(
    ...
    key = (
        ...
        int(slot),  # [07/09] one scratch set per fat stream
    )
...
def apply_exl3_batched_fat(
    ...
    [07/09] EXL3_FAT_STREAMS=n (>1) round-robins the fat experts over n CUDA
    streams with one scratch set each. A fat GEMM at ~128 rows launches only
    16-32 CTAs on 48 SMs and costs a flat ~200 us whatever the row count, so
    overlapping experts is where the time goes (micro-bench: x3.3 at 4 streams).
    Needs the atomicAdd scatter kernel (ext.exl3_fat_scatter_atomic) and the
    direct kernel path; falls back to 1 stream otherwise, and under graph capture.
    """
    ext = load_exllamav3_ext()
    nstreams = fat_streams_requested()
    if nstreams > 1 and not (
        use_kernel
        and hasattr(ext, "exl3_fat_scatter_atomic")
        and not torch.cuda.is_current_stream_capturing()
    ):
        nstreams = 1
    streams = _fat_streams(xh.device, nstreams) if nstreams > 1 else None
    current = torch.cuda.current_stream(xh.device)
    if streams is not None:
        for s_ in streams:
            s_.wait_stream(current)
        _EXL3_FAT_DIAG["multistream_calls"] = _EXL3_FAT_DIAG.get("multistream_calls", 0) + 1
    offset = 0
    i_fat = 0
    for e, n_rows in enumerate(counts_host):
        start = offset
        ...
        i_fat += 1
        if streams is None:
            _fat_expert_step(ext, xh, token_sorted, weight_sorted, start, offset, n_rows,
                             inners[e], limit, out, use_kernel, 0)
        else:
            with torch.cuda.stream(streams[slot]):
                _fat_expert_step(ext, xh, token_sorted, weight_sorted, start, offset, n_rows,
                                 inners[e], limit, out, use_kernel, slot)
    if streams is not None:
        for s_ in streams:
            current.wait_stream(s_)
    return out


def _fat_expert_step(
    ext, xh, token_sorted, weight_sorted, start, offset, n_rows, inner, limit, out, use_kernel, slot
):
    if True:
        token_idx = token_sorted[start:offset]
        gate = inner["gate"]
        up = inner["up"]
        down = inner["down"]
        scratch = _fat_scratch(xh.device, n_rows, gate, slot)
        intermediate = int(gate.out_features)
        ...
        # [07/09] exl3_fat_gemm2 reads gate|up from the two expert trellises directly:
        # no packed13 / svh13 staging copy (was one D2D copy of the whole gate+up
        # trellis per fat expert per chunk, ~5 % of the prefill GPU time).
        use_kernel2 = (
            use_kernel
            and hasattr(ext, "exl3_fat_gemm2")
            and intermediate % 128 == 0
            and os.environ.get("EXL3_FAT_GEMM2", "1") == "1"
        )
        if use_kernel2:
            ext.exl3_fat_gemm2(
                h13, gate.trellis, up.trellis, gate_up, gate.svh, up.svh,
                gate.K, gate.mcg, gate.mul1,
            )
            _EXL3_FAT_DIAG["direct_calls"] += 1
            _EXL3_FAT_DIAG["direct2_calls"] = _EXL3_FAT_DIAG.get("direct2_calls", 0) + 1
        else:
...
class Exl3Config(QuantizationConfig):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # [dense-overlay] mul1 accepté au niveau config : le draft DFlash2
        # converti par exllamav3 1.4 déclare codebook=mul1 (linéaires denses
        # uniquement — le chemin experts, mcg, n'est pas concerné : un draft
        # n'a pas de RoutedExperts).
        if self.codebook not in ("mcg", "mul1"):
            raise ValueError(
                f"this overlay only implements codebook mcg/mul1; got {self.codebook!r}"
            )
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str):
        ...
        # [dense-overlay] lm_head : ParallelLMHead appelle create_weights/apply
        # avec la meme forme d'arguments qu'une linéaire (vocab-parallel =
        # colonne contiguë, tp_rank/tp_size portés par la couche).
        from vllm.model_executor.layers.vocab_parallel_embedding import (
            VocabParallelEmbedding,
        )

        if isinstance(layer, VocabParallelEmbedding) and self._matches_non_routed_exl3(
            prefix
        ):
            return Exl3LinearMethod(self, bits=self._bits_for_non_routed(prefix))
        return None
...
class Exl3LinearMethod(LinearMethodBase):
    ...
    def _make_weight_loader(
        ...
        def weight_loader(
            ...
            # Décision par les shapes : certaines couches (le draft DFlash2)
            # sont construites RÉPLIQUÉES avec des partitions pleine taille
            # mais des attributs tp_rank/tp_size globaux — si le tenseur plein
            # colle déjà à la destination, ne pas narrower.
            _dest_shape = None
            if suffix == "trellis":
                t0 = sum(s // 16 for s in output_partition_sizes[:shard_idx])
                _dest_shape = (
                    param.data.shape[0],
                    output_partition_sizes[shard_idx] // 16,
                    param.data.shape[2],
                )
            elif suffix == "suh":
                _dest_shape = tuple(param.data[shard_idx].shape)
            elif suffix == "svh":
                _dest_shape = (output_partition_sizes[shard_idx],)
            if (_dest_shape is not None
                    and tuple(sharded.shape) != _dest_shape
                    and tuple(loaded.shape) == _dest_shape):
                sharded = loaded
