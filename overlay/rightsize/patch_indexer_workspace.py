...
[nightly port, 2026-09-07] Identical to glm53-dense/overlay/patch_indexer_workspace.py
except the builder anchor: the nightly reads ``kv_cache_spec.tokens_per_state``
instead of ``.compress_ratio``. Sizing site and splitter feed are unchanged
(``compressed_seq_lens_cpu = seq_lens_cpu // compress_ratio`` -> splitter,
workspace = ``max_model_len * 40`` in TOKENS).
